...
```
Question: What patterns do you notice in correlation among these variables?

As is to be expected, the correlation between variables decreases with relative distance. For example, the amount of tree cover in a 5 km radius is highly correlated with the amount of tree cover in a 4 km radius, but not at all with the amount of tree cover in a 100 m radius. 
...
To condense the information into just one or two variables, one could potentially 

If two covariates representing the same ecological features at different scales are NOT statistically correlated with one another, then I would assume including them in the same multiple-linear regression should not be an issue. 
