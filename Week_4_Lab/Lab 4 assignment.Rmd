...
ggplot(ENNData, aes(x = Threshold, y = ENNValues)) +
  ...
  theme_minimal() +
  labs(title = "Mean Nearest-Neighbor Distance vs Threshold",
       x = "Threshold",
...
# Assuming your previous setup for LandscapeCohesion and the linear model

# Adjust the y position for the annotation to be within the visible range
adjusted_y_position_for_label <- 99.5 # Adjust this value as needed

ggplot(LandscapeCohesion, aes(x = HValue, y = CohesionValues)) +
  geom_point() +  # Add points
  geom_line() +   # Connect points with lines (optional)
  geom_smooth(method = "lm", se = TRUE, color = "red") + # Add line of best fit with standard error
  annotate("text", x = 0.5, y = adjusted_y_position_for_label, label = CohesionLineLabel, hjust = 0, vjust = 1, color = "red") +
  coord_cartesian(ylim = c(99, max(LandscapeCohesion$CohesionValues, na.rm = TRUE))) + # Adjust y-axis limits
  theme_minimal() +
  labs(title = "Patch Cohesion vs H Value",
       x = "H",
       y = "Patch Cohesion")
...
ggplot(LandscapeCoreAreaMean, aes(x = HValue, y = CoreAreaValues)) +
  ...
  theme_minimal() +
  labs(title = "Core Area Mean vs H Value",
       x = "H",
       y = "Core Area Mean")
...
NumCoreAreasmodelline_label <- paste("y =", round(intercept, 2), round(slope, 2), "H")
...
ggplot(LandscapeNumCoreAreas, aes(x = HValue, y = CoreAreas)) +
  ...
  theme_minimal() +
  labs(title = "Number of Disjunct Core Areas vs H Value",
       x = "H",
