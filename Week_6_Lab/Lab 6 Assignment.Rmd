...
Yes, upon initial inspection, there does appear to be spatial dependence in the bee response data. Both high and low mean abundance values appear more clustered than what would be expected if the samples were spatially independent from one another.
...
```{r}
# Checking for anistotropy
emp4Geor = variog4(geoBee, max.dist=maxDist, direction = c(0, pi/4, pi/2, 3*pi/4))
plot(emp4Geor) # --> Indicates spatial covariance is similar in all directions
...
#Pure Nugget -->  Model with no spatial correlation, only nugget effect. Useful for baseline comparison.
mlPN = likfit(geoBee, cov.model='pure.nugget', ini=c(7, 60))
summary(mlPN)

# Compare
AIC(mlExp, mlSph, mlPN)

plot(empGeor, envelope=empEnv)
lines(mlExp, col='blue')
```
Explain findings based on AIC values. Of the three models fit, a sphere model appears the most accurate, but doesn't fit super well based on the eye test.
...
FlowerDistribution = focal(floral, w = newraster, fun='mean')
...
# Plot with ggplot2
ggplot() +
  geom_raster(data = FlowerDistribution_df, aes(x = x, y = y, fill = focal_mean)) +
...
blueVanes$meanflowers = flowerextraction$focal_mean
