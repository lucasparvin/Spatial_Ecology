...
mesic_mean_patch_size_sqkm <- (mean(mesic_patch_areas * cell_area))/1000000
...
total_habitat_area_sqkm <- (sum(values(rf_binary) == 1, na.rm = TRUE) * cell_area)/1000000 # Convert square km

# Calculate mean patch size
patch_areas <- table(values(habitat_patches))
mean_patch_size_sqkm <- (mean(patch_areas * cell_area))/1000000
