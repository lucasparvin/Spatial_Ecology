...
**ANSWER:** The the bioclim and Maxent models were left out because these models operate under different assumptions than GLM, GAM, Boosted Regression Trees, and Random Forest models. Bioclim uses an envelope approach, considering environmental minimums, maximums, and using presence data. Maxent, also niche-based, focuses on the probability of species' presence with respect to different environmental variables and might scale differently from the predictions of regression or machine learning models. The ensemble model aims to combine models with similar output scales and interpretations, ensuring that the weighted average is meaningful and consistent across the landscape. Thus, combining models that scale differently or represent slightly different things would make the ensemble model difficult to interpret.
...
# Confined to 6 columns, so I omitted bioclim model
summaryEval = summaryEval %>% 
  setNames(c('model', 'auc', 'corr', 'll', 'threshold', 'sens', 'spec', 'tss', 'kappa')) %>% 
...
**ANSWER:** Yes, I would argue that the ensemble model is better than at least one of the other models. Its AUC value is higher than all but one of the models compared here. Only the GAM model performs better in that metric. The same pattern is observed in regards to the TSS. The ensemble model also has the highest Kappa value.
...
**ANSWER:** The AUC value indicates that the presence-absence model is better. The TSS also indicates that the presence-absence model is better. In contrast, the Kappa statistic indicates that the presence-background model has higher agreement than the presence-absence model. Ultimately, while neither model appears to be very good, the presence-absence model seems to be stronger, indicated by 2 of the 3 statistics focused on in this challenge. Furthermore, intuitively, I would guess that a model fit with true absences would make for a better model, if the absences are not severely biased. 
...
#nFolds = 5
#kfoldPres = kfold(presCovs, k=nFolds) # for both models
#kfoldBack = kfold(backCovs, k=nFolds) # for back model
#kfoldAbs = kfold(absCovs, k=nFolds) # for abs model

#for(i in 1:nFolds){
 # valPres = presCovs[kfoldPres==i,]
 # valBack = backCovs[kfoldBack==i,]
 # valAbs = absCovs[kfoldAbs==i,]
 # valBothBack = rbind(valPres, valBack)
 # valBothAbs = rbind(valPres, valAbs)
 # trainPres = presCovs[kfoldPres!=i,]
 # trainBack = backCovs[kfoldBack!=i,]
 # trainAbs = absCovs[kfoldAbs!=i,]
 # trainBothBack = rbind(trainPres, trainBack)
 # trainBothAbs = rbind(trainPres, trainAbs)
 # glmModelBack = glm(pres ~ canopy + elev + I(elev^2) + mesic1km + precip, family='binomial', data=trainBothBack)
 # glmModelAbs = glm(pres ~ canopy + elev + I(elev^2) + mesic1km + precip, family='binomial', data=trainBothAbs)
  #AUC
  #aucBack = auc(valBothBack, which.model = i)
  #True skill statistic
  # tss = sens$sensitivity + spec$specificity - 1
#}
```

**ANSWER:** Told not to complete.
