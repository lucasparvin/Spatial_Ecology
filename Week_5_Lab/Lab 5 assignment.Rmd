...
**Overall, these findings hint that #1 critters are aggregating, while #2 critters are demonstrating more random levels of aggregation. Given that Ripley's L values plummet for both critters, I expect this is a function of not correcting for edge effects. For example, for any given critter 5 units away from an edge, you would expect the level of aggregation to decrease as you expand past 5 units. This can be attributed to the fact that despite expanding your radius, you wouldn't be increasing the number of points to interact with on the side of the nearest edge(s), thus introducing bias.**
...
2. Ripley's L values for #1 critters appear pretty different when using isotropic vs toroidal shift corrections, while the values for #2 critters appear nearly identical across both correction types. At first, I found this odd, so I dove a bit deeper into how these corrections actually function. While isotropic correction weighs points differently depending on their proximity to an edge, a toroidal shift artificially creates a continuous landscape, using opposite edges of the window to continue the distribution of points. In our scenario, I find it more appropriate to use an isotropic correction, because provided a more extensive landscape (toroidal shift), it is likely that our prey species would not behave precisely the same way. In our simulation, the prey species is repelled by predators AND confined by edges. Thus, we may see greater clustering near edges, which appears to be the case. A toroidal shift would repeat/project these clusters, which originally formed near edges, across a greater landscape with edges that are theoretically farther away. In short, a toroidal shift is less representative of the true patterns exhibited by our prey critters than an isotropic correction, whereas for our predators, it should make little difference if they are randomly dispersed. 
...
While I am feeling pretty confident that #1 critters are the prey and #2 critters are the predators, let's continue our analysis.
...
After our comparison to CSR, it is clear that critter 1, across ~4-21 units, aggregates more strongly than what should reasonably be expected of a random distribution. This is not the case for critter 2, which does not exhibit significant differences from CSR. Now we can confidentially infer that #1 critters are the prey, given that they are aggregating, which can be anticipated due to shared repulsion from predators. However, let's take a look at how they directly respond to #2 critters, the expected predators.
