...
## I think this error is occurring due to the fact that there are NLCD values of developed, open space, etc. in the "covariates" object, but these cover types are absent from the INBU_and_Covs data frame, from which the GLMs are built. Thus, I have to either remove pixels with these cover types from the get-go OR figure out a way to just NOT predict to these cover types in glmMap. If I remove these pixels from the get-go, I will need to align the elevation, slope, and aspect rasters to the NLCD raster (essentially punching holes in those rasters) in order for each raster to stack properly. What is the best way to approach this? How can I do it?
...
for(i in 1:folds){
 ...
 # Models
  ...
  # Kappa
  # kappaValues[i,1] = as.numeric(PresenceAbsence::optimal.thresholds(valDataFrame, which.model = 1, opt.methods = 4))
  # kappaValues[i,2] = as.numeric(PresenceAbsence::optimal.thresholds(valDataFrame, which.model = 2, opt.methods = 4))
  # kappaValues[i,3] = as.numeric(PresenceAbsence::optimal.thresholds(valDataFrame, which.model = 3, opt.methods = 4))
  # kappaValues[i,4] = as.numeric(PresenceAbsence::optimal.thresholds(valDataFrame, which.model = 4, opt.methods = 4))
  
  # Kappa
  kappaOpt = as.numeric(PresenceAbsence::optimal.thresholds(valDataFrame, which.model = i, opt.methods=3))
  kappaValues[i, 1] = as.numeric(PresenceAbsence::Kappa(cmx(valDataFrame, which.model = i, threshold = kappaOpt[[2]])))
  
  
  #   kappaOpt = optimal.thresholds(valDataPA, which.model = 1, opt.methods=3)
#   kappaPA[i] = Kappa(cmx(valDataFrame, which.model = 1, threshold = kappaOpt[[2]]))
...
#   
# 
#   glmPresAbs = glm(INBU ~ elevation + treecover + slope, family = 'binomial', data=presAbsCovs)
#   
#   glmPresBack = glm(INBU ~ elevation + treecover + slope, family = 'binomial', data=presBackCovs)
#   
#   valDataPA = data.frame('ID' = 1:nrow(valPresAbs)) %>% 
#   mutate(obs = valPresAbs$pres, glmPAVal = predict(glmPresAbs, valPresAbs %>% 
#                                                    dplyr::select(elevation:treecover),
#                                                  type = 'response'))
#   
...
# NEED TO USE kfold to calculate validation statistics
```{r}
...
#   
#   glm_elevtreeslope2 = glm(INBU ~ elevation + treecover + slope, family = 'binomial', data=trainBoth)
# 
#   valData = data.frame('ID' = 1:nrow(valPres)) %>% 
#   mutate(obs = valPres$INBU,
#          glmVal = predict(glm_elevtreeslope2, valPres %>% dplyr::select(elevation:treecover), type='response'))
#   
# boyceVals[i] = ecospat.boyce(fit = glmMap, obs=valData[,3], res=100, PEplot=F)$cor
# 
# }
# 
# glmBV <- mean(boyceVals)
```
...
#   trainBoth = rbind(trainPres, trainBack)
#   
# gam_elevtreeslope2 = gam(INBU ~ s(treecover, k=3) + s(elevation, k=3) + s(slope, k=3), family='binomial', data = trainBoth, method='ML')
# 
#   valData = data.frame('ID' = 1:nrow(valPres)) %>% 
#   mutate(obs = valPres$INBU,
#          gamVal = predict(gam_elevtreeslope2, valPres %>% dplyr::select(elevation:treecover), type='response'))
#   
# boyceVals[i] = ecospat.boyce(fit = gamMap, obs=valData[,3], res=100, PEplot=F)$cor
# 
# }
# 
# gamBV <- mean(boyceVals)
```


# Validating our Random Forest Model
```{r}
# set.seed(2024)
# boyceVals = rep(NA, nFolds)
# 
...
#   trainBoth = rbind(trainPres, trainBack)
# 
#  tuneRF(y = as.factor(trainBoth$INBU), x=trainBoth[, c(1:3)], stepFactor = 2, ntreeTry = 500)
# # mtry 1 is best
# 
# rfModel2 = randomForest(as.factor(INBU) ~ elevation + treecover + slope, data=trainBoth, mtry=1, ntree=500, na.action = na.omit)
# 
# valData = data.frame('ID' = 1:nrow(valPres)) %>%
#   mutate(obs = valPres$INBU,
#          rfVal = predict(rfModel2, valPres %>% dplyr::select(elevation, treecover, slope), type='response'))
# 
# 
# boyceVals[i] = ecospat.boyce(fit = rfMap, obs=valData[,3], res=100, PEplot=F)$cor
# 
# }
# 
# rfBV <- mean(boyceVals)
...
# Validating our Boosted Regression Tree
...
#   trainBoth = rbind(trainPres, trainBack)
#   
# boostModel2 = gbm(INBU ~ elevation + treecover + slope, distribution='bernoulli', n.trees=100, interaction.depth=2, shrinkage=0.1, bag.fraction=0.5, data=INBU_and_Covs)
# 
#   valData = data.frame('ID' = 1:nrow(valPres)) %>% 
#   mutate(obs = valPres$INBU,
#          gamVal = predict(boostModel2, valPres %>% dplyr::select(elevation:treecover), type='response'))
#   
# boyceVals[i] = ecospat.boyce(fit = boostMap, obs=valData[,3], res=100, PEplot=F)$cor
# 
# }
# 
# boostBV <- mean(boyceVals)

```
# Validating our Maxent Model
```{r}
...
#   trainBoth = rbind(trainPres, trainBack)
# 
# pbVect2 = trainBoth$INBU
# covs2 <- trainBoth %>% dplyr::select(c('elevation', 'treecover', 'slope'))
# 
# maxentModel2 = maxnet(p = pbVect2,
#                      data= covs2,
#                      regmult = 1,
#                      classes='lqpht')
# 
#   valData = data.frame('ID' = 1:nrow(valPres)) %>%
#   mutate(obs = valPres$INBU,
#          maxentVal = predict(maxentModel2, valPres %>% dplyr::select(elevation:treecover), type='logistic'))
# 
# boyceVals[i] = ecospat.boyce(fit = maxentMap, obs=valData[,3], res=100, PEplot=F)$cor
# 
# }
# 
# maxentBV <- mean(boyceVals)
...
# Boyce_Values_Table <- data.frame(
#   Model = c("glm", "gam", "rf", "boost", "maxent"),
#   bv = c(glmBV, gamBV, rfBV, boostBV, maxentBV))
# 
# print(Boyce_Values_Table)
