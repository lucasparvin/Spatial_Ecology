...
# Convert 'Class' to a factor if it's not already
nlcd_df$Class <- factor(nlcd_df$Class)
...
#######################
# Define the values to be converted to NA
values_to_na <- c("Developed, Low Intensity", "Developed, Medium Intensity", "Developed, Open Space")

# Convert matching values to NA
nlcd_df$Class[nlcd_df$Class %in% values_to_na] <- NA
...
# GAMs
```{r}
gam_all = gam(INBU ~ s(NLCD, k=3) + s(elevation, k=3) + s(slope, k=3) + s(aspect, k=3), family='binomial', data = INBU_and_Covs, method='ML')
summary(gam_all)

gamMap = predict(covariates, gam_all, type='response')
...
```{r}
set.seed(2024)
boostModel = gbm(INBU ~ elevation + NLCD + slope + aspect, distribution='bernoulli', n.trees=100, interaction.depth=2, shrinkage=0.1, bag.fraction=0.5, data=INBU_and_Covs)
...
tuneRF(y = as.factor(INBU_and_Covs$INBU), x=INBU_and_Covs[,1:4], stepFactor = 2, ntreeTry = 500)

# mtry 1 is best

rfModel = randomForest(as.factor(INBU) ~ NLCD + elevation + slope + aspect, data=INBU_and_Covs, mtry=1, ntree=500, na.action = na.omit)
...
covs <- INBU_and_Covs %>% dplyr::select(elevation:NLCD)
