...
ggplot(treecover_df, aes(x = x, y = y, fill = treecover)) +
   ...
   coord_fixed() +
   theme_minimal() +
   labs(title = "Tree Canopy Cover within Perdido Area")
...
#gam_elevtreeslope = gam(INBU ~ s(treecover, bs = "cr", k = 10) + s(elevation, bs = "cr", k = 10) + s(slope, bs = "cr", k = 10), family = 'binomial', data = INBU_and_Covs, method = 'ML')
...
# Creating the Kappa plot
kappaPlot <- ggplot(resultsTable, aes(x = Model, y = Mean_Kappa, fill = Model)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_hline(yintercept = 0.2, linetype = "dotted", color = "black", size = 1) +
  geom_hline(yintercept = 0.6, linetype = "dotted", color = "black", size = 1) +
  annotate("text", x = 1, y = 0.2, label = "Slight Agreement", vjust = -0.5, hjust = 0.5) +
